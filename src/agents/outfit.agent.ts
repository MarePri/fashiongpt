...
const COLOR_GROUPS: Record<string, string[]> = {
  ...
  pastels: ['Blush', 'Dusty Pink', 'Cream', 'Ivory'],
  brights: ['Gold', 'Silver', 'Cobalt', 'Red'],
...
function computeColorHarmony(products: Product[]): number {
  if (products.length < 2) return 80;
  const itemGroups = products.map(p => getColorGroup(p.color));
  let harmonyCount = 0;
  let pairCount = 0;
  ...
  return Math.round(60 + (harmonyCount / pairCount) * 35);
}
